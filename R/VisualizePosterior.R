...
plotChrEnichment = function(expmat,chr,normFactor,gene_positions,n=1,groups1=NULL,groups2=NULL,start=NULL,end=NULL,k=2,vis=T,postProb=0.95,repetitions=5,gene_pos_index=2){
  par(mfrow=c(2,2))
  if (!is.null(groups1)){
  ...
  if (!is.null(start)){
    chr_genes=gene_positions[which(gene_positions[,3]==chr & gene_positions[,4]>start &  gene_positions[,5]<end),gene_pos_index]
  }
  else{
    chr_genes=gene_positions[which(gene_positions[,3]==chr),gene_pos_index]
  }
...
plotAll = function (mat,normFactor,regions,gene_pos,fname,normal=NULL,tumor=NULL,postProb=0.8,repetitions=4,gene_pos_index=2){
  ...
  for(i in 1:nrow(regions)){
    mixmdl=plotChrEnichment(mat,regions[i,1],normFactor,gene_pos,nrow(regions),normal,tumor,regions[i,2],regions[i,3],postProb=postProb,repetitions=repetitions, gene_pos_index=gene_pos_index)
    if (!is.null(mixmdl)){
