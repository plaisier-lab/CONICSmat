...
getGenePositions= function(gene_names,ensembl_version="dec2016.archive.ensembl.org",species="human", filter="hgnc_symbol"){
  if (species=="human"){
	ensembl = biomaRt::useMart(biomart = "ENSEMBL_MART_ENSEMBL", dataset = "hsapiens_gene_ensembl", host=ensembl_version)
	gene_positions <- biomaRt::getBM(attributes=c('ensembl_gene_id','hgnc_symbol','chromosome_name','start_position','end_position'), filters =filter, values =gene_names, mart = ensembl)
  }
  else {
